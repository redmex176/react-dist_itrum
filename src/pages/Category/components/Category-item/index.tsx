...
const CategoryItem: React.FC<CategoryItemProps> = ({
  ...
  const elements = categories.map((item) => (
    <li
      className={`${styles.title__item} ${item.isActive ? styles.active : ''}`}
      key={item.id}
      ...
      {isEdit && editedCategoryId === item.id ? (
        <>
          <input
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) => setInputEditValue(e.target.value)}
            onKeyDown={(e: KeyboardEvent<HTMLInputElement>) => {
              if (e.key === 'Enter') {
                isSubCategory ? handleSaveSubCategoryEdit(inputEditValue) : handleSaveEdit(inputEditValue);
              } else if (e.key === 'Escape') {
                handleCancelEdit();
              }
            }}
          />
          <SaveSvg onClick={() => isSubCategory ? handleSaveSubCategoryEdit(inputEditValue) : handleSaveEdit(inputEditValue)} />
          <CloseSvg onClick={handleCancelEdit} />
        </>
      ) : (
        <>
          <p onDoubleClick={() => isSubCategory ? handleEditSubCategory(item.id) : handleEditCategory(item.id)}>
            {!isSubCategory ? item.category : item.subCategory}
          </p>
          <EditSvg onClick={() => isSubCategory ? handleEditSubCategory(item.id) : handleEditCategory(item.id)} />
  ...
  return (
    <div className={styles.card}>
      <input
        ...
        onChange={(e: ChangeEvent<HTMLInputElement>) => isSubCategory ? setInputSubAddValue(e.target.value) : setInputAddValue(e.target.value)}
        onKeyDown={(e: KeyboardEvent<HTMLInputElement>) => {
          if (e.key === 'Enter') {
      ...
      />
      <button onClick={isSubCategory ? handleAddSubCategory : handleAddCategory}>
            <p>{btnText}</p>
      </button>
      {elements.length === 0 ? (
        <div className={styles.title__items}>
          {isSubCategory ? 'Здесь пока нет подкатегорий' : 'Здесь пока нет категорий'}
        </div>
      ) : (
        <>
          <div className={styles.title__card}>Название {isSubCategory ? 'подкатегории' : 'категории'}</div>
          <ul>{elements}</ul>
        </>
      )}
    </div>
